...
void MAX17260::poll(){ //update dynamic variables, must be called periodically
	...
	rawavgCurrent=this->read(avgCurrent_Reg);
	rawcoulombCounter=this->read(coulombCounter_Reg);
	rawavgVoltage=this->read(avgVcell_Reg);
	rawTemp=this->read(Temp_Reg);
	rawavgTemp=this->read(avgTemp_Reg);
	...
	avgCurrent=rawavgCurrent*Istep;
	coulombCounter=rawcoulombCounter*Capacitystep;
	avgVoltage=rawavgVoltage*Vstep;
	Temp=rawTemp*Tempstep;
	avgTemp=rawavgTemp*Tempstep;
}
...
void Cargadescargador::setMode(uint8_t _mode){
		mode=_mode;
	  	switch(mode){
	  	case 0:{
